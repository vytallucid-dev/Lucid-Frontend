...
import { FlowTrackerMiniChart } from "./FlowTrackerMiniChart";
import { flowChartCategories } from "./flowChartHelpers";
...
export function FlowTrackerView({ onClose }: FlowTrackerViewProps) {
  ...
  const { data, isLoading, error, refetch } = useQuery({
    queryKey: customRange
      ? ["nifty", "flow-tracker", "custom", customRange.from, customRange.to]
      : ["nifty", "flow-tracker", timeframe.limit],
    queryFn: () =>
      customRange
        ? loadFlowTrackerData({ from: customRange.from, to: customRange.to })
        : loadFlowTrackerData({ limit: timeframe.limit }),
  });
  ...
  // Shared x-axis categories so the primary + mini charts stay pixel-aligned.
  const categories = useMemo(() => (data ? flowChartCategories(data.points) : []), [data]);
          ...
          {isLoading ? (
            ...
            <>
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <TimeframeControl
                  presets={TIMEFRAMES.map((tf) => ({ key: String(tf.limit), label: tf.label }))}
                  activePresetKey={customRange ? null : String(timeframe.limit)}
                  onSelectPreset={(key) => {
                    const tf = TIMEFRAMES.find((t) => String(t.limit) === key);
                    if (tf) {
                      setCustomRange(null);
                      setTimeframe(tf);
                    }
              ...
              {/* Secondary context panels — DII absorption / FII long %, own scale each */}
              {(showDii || showInd13) && (
                <div className={`grid grid-cols-1 ${showDii && showInd13 ? "sm:grid-cols-2" : ""} gap-4`}>
                  {showDii && (
                    <div className="lt-card p-4" style={{ height: 140 }}>
                      <FlowTrackerMiniChart
                        points={data.points}
                        categories={categories}
                        seriesKey="diiAbsorption"
                        onHoverIndex={setHoverIndex}
                      />
                    </div>
                  )}
