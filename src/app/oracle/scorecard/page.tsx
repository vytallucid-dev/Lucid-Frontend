...
function IndicatorRow({ ind }: { ind: ScorecardIndicator }) {
  const scoreColor =
    ind.score > 0 ? "#10B981" : ind.score < 0 ? "#EF4444" : "#64748B";
  const scoreBg =
    ind.score > 0
      ? "rgba(16, 185, 129, 0.15)"
      : ind.score < 0
  ...
  return (
    ...
    >
      <td className="py-2.5 px-3 text-[13px] font-medium" style={{ color: "#F1F5F9" }}>
...
function SectionCard({ section }: { section: ScorecardSection }) {
  const subtotalBias = getBias(section.subtotal);
  return (
      ...
      <table className="w-full">
        <thead>
          <tr style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
            <th className="label text-left px-3 py-2" style={{ color: "#64748B" }}>
              INDICATOR
            </th>
            <th className="label text-right px-3 py-2" style={{ color: "#64748B" }}>
              ACTUAL
            </th>
            <th className="label text-right px-3 py-2" style={{ color: "#64748B" }}>
              FORECAST
...
export default function ScorecardPage() {
  const [selectedKey, setSelectedKey] = useState<ScorecardAssetKey>("USD");
  const asset = scorecardAssets.find((a) => a.key === selectedKey)!;

  const biasColor = getBiasColor(asset.bias);
  const scoreColor = getScoreColor(asset.totalScore);
      ...
      <div className="flex gap-6 items-start">
        ...
        <div className="w-[280px] shrink-0 sticky top-[112px] flex flex-col gap-4">
          ...
          <div className="glass-card p-5 text-center">
            <ScoreGauge score={asset.totalScore} />
            <div className="mb-2 -mt-1">
              <span
                className={`inline-block px-3 py-1 rounded-full text-xs font-semibold ${getBiasPillClass(asset.bias)}`}
              >
                {asset.bias}
              </span>
            </div>
